...
function toolRadioBtnOnClick(action: number) {
    selectedTool = action;
}

function componentDivAction(action: number) {
    stopSimulation();

    let selectedComponentParentId = circuit.getComponentParentById(selectedComponent.id).id;
    if(action == 1) {
...
function circuitDivAction(action: number) {
    stopSimulation();
...
function componentEditAction(action: number) {
    stopSimulation();
...
function updateComponentInfoDiv(component: Component) {
    if(component != null && componentInfoDiv.style.display != "none") {
        ...
        if(component.hasFailed) getElement("componentWarningText").innerHTML = "Rikki, suurin sallittu virta ylittyi!";
